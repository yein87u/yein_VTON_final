...
class MambaLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        x_mamba = checkpoint(self.mamba, x_norm, use_reentrant=False) + self.skip_scale * x_flat
...
class ResMambaBlock(nn.Module):
 #2, 8|16|32|64, '', ("RELU", {"inplace": True})
    def __init__(
        self,
        spatial_dims: int,      # 2
        in_channels: int,       # 8|16|32|64
        norm: tuple | str,      # ''
        kernel_size: int = 3,
...
class LightMUNet(nn.Module):
    ...
    def __init__(
        ...
        self.norm = norm
        self.upsample_mode = UpsampleMode(upsample_mode)    # 僅拉大影像尺寸
    ...
    def _make_down_layers(self):
        ...
        for i, item in enumerate(blocks_down):
            layer_in_channels = filters * 2**i  # i = 0|1|2|3, layer_in_channels = 8|16|32|64
            downsample_mamba = (
                get_mamba_layer(spatial_dims, layer_in_channels // 2, layer_in_channels, stride=2)
                if i > 0
                else nn.Identity()
            )
            # 2, 8|16|32|64, '', ("RELU", {"inplace": True})                for 1|2|2|4
    ...
    def _make_up_layers(self):
        ...
        for i in range(n_up):
            ...
            up_layers.append(
                nn.Sequential(
                    *[
                        ResUpBlock(spatial_dims, sample_in_channels // 2, norm=norm, act=self.act)  # in_channel = 32|16|8
                        for _ in range(blocks_up[i])    #1|1|1
                    ]
                )
            )
            up_samples.append(
