...
class ChannelAttentionModule(nn.Module):
    def __init__(self, in_channels, reduction=4):
        ...
        self.fc = nn.Sequential(
            ...
            nn.ReLU(inplace=True), # 小於 0 的數值設為 0
    ...
    def forward(self, x):
        ...
        # [  2,   8,  64,  48]
        # [  2,  16,  32,  24]
        # avg 對每個 channel 的整個 H×W 區域做平均；max 在每個 channel 中整個 H×W 區域找出最大值
        avg_out = self.fc(self.avg_pool(x))
        max_out = self.fc(self.max_pool(x))
        '''               avg/max pool              fc conv1      ReLU      fc conv2                  '''
        # [  2,   4, 128,  96] => [  2,   4,   1,   1] => [  2,   1,   1,   1] => [  2,   4,   1,   1]
        # [  2,   8,  64,  48] => [  2,   8,   1,   1] => [  2,   2,   1,   1] => [  2,   8,   1,   1]
        # [  2,  16,  32,  24] => [  2,  16,   1,   1] => [  2,   4,   1,   1] => [  2,  16,   1,   1]

        out = avg_out + max_out  # (穩定+敏感)資訊，達到互補，讓模型能更穩定學習重要的channel
        return self.sigmoid(out) # 限縮數值[0, 1]

class SpatialAttentionModule(nn.Module):
    def __init__(self, kernel_size=7):
        super(SpatialAttentionModule, self).__init__()
        self.conv1 = nn.Conv2d(2, 1, kernel_size, padding=kernel_size//2, bias=False)   # conv(2, 1, 7, 3)
        self.sigmoid = nn.Sigmoid()
...
class FusionConv(nn.Module):
    def __init__(self, in_channels, out_channels, factor=4.0):
        ...
        # FusionConv(112, 64)  /  FusionConv(112, 128)  /  FusionConv(112, 256) 
        dim = int(out_channels // factor)
        #  16/32/64
        self.down = nn.Conv2d(in_channels, dim, kernel_size=1, stride=1)        # conv(     112, 16/32/64, 1, 1)
        self.conv_3x3 = nn.Conv2d(dim, dim, kernel_size=3, stride=1, padding=1) # conv(16/32/64, 16/32/64, 3, 1, 1)
        self.conv_5x5 = nn.Conv2d(dim, dim, kernel_size=5, stride=1, padding=2) # conv(16/32/64, 16/32/64, 5, 1, 2)
        self.conv_7x7 = nn.Conv2d(dim, dim, kernel_size=7, stride=1, padding=3) # conv(16/32/64, 16/32/64, 7, 1, 3)
        self.spatial_attention = SpatialAttentionModule()                       
        # [conv(2, 1, 3), sigmoid()]
        self.channel_attention = ChannelAttentionModule(dim)
        # [conv( 16,  1,  1), ReLU(), conv( 1, 16,  1)] /
        # [conv( 32,  2,  1), ReLU(), conv( 2, 32,  1)] /
        # [conv( 64,  4,  1), ReLU(), conv( 4, 64,  1)]

        self.up = nn.Conv2d(dim, out_channels, kernel_size=1, stride=1)         # conv(16/32/64, 64/128/256, 1)

    def forward(self, x1, x2, x4):
        '''B, C, H, W'''
        # ([  2,  16, 128,  96], [  2,  32, 128,  96], [  2,  64, 128,  96])
        # ([  2,  32,  64,  48], [  2,  16,  64,  48], [  2,  64,  64,  48])
        # ([  2,  64,  32,  24], [  2,  32,  32,  24], [  2,  16,  32,  24])
        ...
        # [  2,  32,  64,  48], dim = 32
        # [  2,  64,  32,  24], dim = 64
        ''' 保留重要通道, 抑制不重要通道, x_fused_c是每個channel的注意力值[0 ~ 1] '''
        x_fused_c = x_fused * self.channel_attention(x_fused)
        # [  2, 16, 128,  96] = [  2,  16, 128,  96] * [  2,  16,   1,   1]
        # [  2, 32,  64,  48] = [  2,  32,  64,  48] * [  2,  32,   1,   1]
        # [  2, 64,  32,  24] = [  2,  64,  32,  24] * [  2,  64,   1,   1]

        ''' 根據注意力值提取多尺度的特徵, 用不同 kernel size 掃過影像獲取特徵 '''
        x_3x3 = self.conv_3x3(x_fused)  # [  2, 16, 128,  96]
        x_5x5 = self.conv_5x5(x_fused)  # [  2, 32,  64,  48]
        x_7x7 = self.conv_7x7(x_fused)  # [  2, 64,  32,  24]
        ...
        # [  2, 32,  64,  48]
        # [  2, 64,  32,  24]
        ''' 將影像壓縮成一片通道, 以此來產生每個像素對於影像的重要性 '''
        x_fused_s = x_fused_s * self.spatial_attention(x_fused_s)
        # [  2, 16, 128,  96] = [  2,  16, 128,  96] * [  2,  1, 128,  96], dim = 16
        # [  2, 32,  64,  48] = [  2,  32,  64,  48] * [  2,  1,  64,  48], dim = 32
        # [  2, 64,  32,  24] = [  2,  64,  32,  24] * [  2,  1,  32,  24], dim = 64

        # 融合多尺度、空間資訊
        x_out = self.up(x_fused_s + x_fused_c)
        # [  2,  64, 128,  96] = [  2,  16, 128,  96] + [  2,  16, 128,  96], dim = 16
        # [  2, 128,  64,  48] = [  2,  32,  64,  48] * [  2,  32,  64,  48], dim = 32
        # [  2, 256,  32,  24] = [  2,  64,  32,  24] * [  2,  64,  32,  24], dim = 64
...
class UpsampleConv(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.conv(x)      # 使用卷積來改變 channel 數量
        return x

class VSSM(nn.Module):
    def __init__(self, patch_size=4, in_chans=36, num_classes=4, depths=[2, 2, 9, 2], 
        ...
        self.backbone = timm.create_model("resnet18", features_only=True, output_stride=32,
                                  out_indices=(1, 2, 3, 4), pretrained=True) # , pretrained_cfg_overlay=dict(file='~/.cache/huggingface/hub/models–timm–resnet18.a1_in1k/pytorch_model.bin'))        # build decoder layers
        base_dims = 64
        
        # model = timm.create_model("resnet18", pretrained=True)
        torch.save(self.backbone.state_dict(), "/mnt/c/Users/User/Desktop/yein_VTON/checkpoints/resnet18.pth")
        state_dict = torch.load('/mnt/c/Users/User/Desktop/yein_VTON/checkpoints/resnet18.pth')
        msg = self.backbone.load_state_dict(state_dict, strict=False)
        print('[INFO] ', msg)
        self.backbone.conv1 = torch.nn.Conv2d(36, 64, kernel_size=(7, 7), stride=(2, 2), padding=(3, 3), bias=False)

        if isinstance(dims, int):
            dims = [int(dims * 2 ** i_layer) for i_layer in range(self.num_layers)]
        
        self.embed_dim = dims[0]                # 64
        self.num_features = dims[-1]            # 512
        self.num_features_up = int(dims[0] * 2) # 128, decoder起始維度
        self.dims = dims                        # [64, 128, 256, 512]
        ...
        for i_layer in range(self.num_layers):
            ...
            # [nn.Identity(),         # i_layer = 0
            #  nn.Linear(512, 256),   # i_layer = 1
            #  nn.Linear(256, 128),   # i_layer = 2
            #  nn.Linear(128, 64)]    # i_layer = 3
            
            if i_layer == 0:
                layer_up = nn.Sequential(
                    VSSLayer(
                        dim = int(dims[0] * 2 ** (self.num_layers - 1 - i_layer)),          # 512
                        depth=2,
                        d_state=math.ceil(dims[0] / 6) if d_state is None else d_state,     # 16
                        drop=drop_rate,                                                     # 0.
                        attn_drop=attn_drop_rate,                                           # 0.
                        drop_path=dpr[sum(depths[:-1]):sum(depths[:])],                     # drop node
                        norm_layer=norm_layer,                                              # nn.LayerNorm
                        downsample=None,
                ...
                layer_up = VSSLayer_up(
                    dim= int(dims[0] * 2 ** (self.num_layers-1-i_layer)),           # 256, 128, 64
                    depth=depths[(self.num_layers-1-i_layer)],                      # 2, 1, 0
                    d_state=math.ceil(dims[0] / 6) if d_state is None else d_state, # 16
                    drop=drop_rate,                                 # 0.
                    attn_drop=attn_drop_rate,                       # 0.
                    drop_path=dpr[sum(depths[:(self.num_layers-1-i_layer)]):sum(depths[:(self.num_layers-1-i_layer) + 1])], # drop node
                    norm_layer=norm_layer,                          # nn.LayerNorm
                    upsample=PatchExpand if (i_layer < self.num_layers - 1) else None,
                    use_checkpoint=use_checkpoint,                  # None
                    layer=i_layer,                                  # 1 ~ 3
                )
        ...
        hidden_dim = int(base_dims // 4)            # 64//4 = 16
        self.AFFs = nn.ModuleList([            
            MSAA(hidden_dim * 7, base_dims),        # 16*7 = 112, 64  
            MSAA(hidden_dim * 7, base_dims * 2),    # 16*7 = 112, 128 = 64*2
            MSAA(hidden_dim * 7, base_dims * 4),    # 16*7 = 112, 256 = 64*4 
        ])
        
        
        self.transfer = nn.ModuleList(
            [
                nn.Conv2d(base_dims, hidden_dim, 1, bias=False),            #  64, 16
                nn.Conv2d(base_dims * 2, hidden_dim * 2, 1, bias=False),    # 128, 32 
                nn.Conv2d(base_dims * 4, hidden_dim * 4, 1, bias=False),    # 256, 64
            ]
        )

        self.upsample = nn.ConvTranspose2d(4, 4, kernel_size=4, stride=2, padding=1)
    ...
    def up_x4(self, x, h, w):
        ...
        cv_img = (x[0].permute(1, 2, 0).detach().cpu().numpy()+1)/2
        rgb = (cv_img*255).astype(np.uint8)
        bgr = cv2.cvtColor(rgb, cv2.COLOR_RGB2BGR)
        os.makedirs('sample/checkimage/', exist_ok=True)
        cv2.imwrite('sample/checkimage/1.jpg', bgr)

        # print("debug: ", x.shape)
        x = self.upsample(x)
        x = self.upsample(x)

        # # 初始化 UpsampleConv，第一次將 4 -> 4
        # up1 = UpsampleConv(4, 4, scale_factor=2).to(device)
        # # 第二次將 4 -> 4
        # up2 = UpsampleConv(4, 4, scale_factor=2).to(device)

        # # 確保輸入在正確的設備上
        # x = x.to(device)

        # # 第一次上採樣，channel 會從 4 -> 4
        # x = up1(x)
        # # 第二次上採樣，channel 會從 4 -> 4
        # x = up2(x)
        return x
    ...
    def forward_downfeatures(self, x_downsample):
        ...
        #  [  2,  32,  24, 256], 
        #  [  2,  16,  12, 512]] = x
        x_down_last = x_downsample[-1] # just x
        x_downsample_2 = x_downsample
        x_downsample = []
        # 除了最後一個元素，全部x_downsample改變維度順序
        for idx, feat in enumerate(x_downsample_2[:-1]):
            ...
            #  [  2, 128,  64,  48], 
            #  [  2, 256,  32,  24]]
            feat = self.transfer[idx](feat) # 0, 1, 2
            '''B, C, H, W'''
            # [[  2,  64, 128,  96] => [[  2,  16, 128,  96], 
            #  [  2, 128,  64,  48] =>  [  2,  32,  64,  48], 
            #  [  2, 256,  32,  24]] => [  2,  64,  32,  24]]
        ...
        # [  2,  64, 128,  96] = self.AFFs[0]([  2,  16, 128,  96], 
        #                                    [  2,  32, 128,  96], 
        #                                    [  2,  64, 128,  96])
        x_down_3 = self.AFFs[1](x_downsample[1], x_down_2_3, x_down_4_3)
        # [  2, 128,  64,  48] = self.AFFs[1]([  2,  32,  64,  48],
        #                                    [  2,  16,  64,  48],
        #                                    [  2,  64,  64,  48])
        x_down_4 = self.AFFs[2](x_downsample[2], x_down_3_4, x_down_2_4)
        # [  2, 256,  32,  24] = self.AFFs[2]([  2,  64,  32,  24],
        #                                    [  2,  32,  32,  24],
        #                                    [  2,  16,  32,  24])

        ''' B, C, H, W => B, H, W, C'''
        x_down_2 = torch.permute(x_down_2, (0,2,3,1))   # [  2, 128,  96,  64]
        x_down_3 = torch.permute(x_down_3, (0,2,3,1))   # [  2,  64,  48, 128]
        x_down_4 = torch.permute(x_down_4, (0,2,3,1))   # [  2,  32,  24, 256]

        # [[  2, 128,  96,  64], [  2,  64,  48, 128], [  2,  32,  24, 256], [  2,  16,  12, 512]]
    ...
    def forward(self, x):
        b, c, h, w = x.size() # 512*384
        # x,x_downsample = self.forward_features(x)
        x, x_downsample = self.forward_resnet(x) # B, H, W, C
        # x = [  2,  16,  12, 512]
        x_downsample = self.forward_downfeatures(x_downsample)
        # x_downsample = [[  2, 128,  96,  64], [  2,  64,  48, 128], [  2,  32,  24, 256], [  2,  16,  12, 512]]
        if self.training:
