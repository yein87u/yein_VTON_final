...
# 將 input 數組中的數據按照一定的規則重排到 output 數組中，需要對特徵圖進行格式轉換或填充的情況下會使用到。
...
# 計算兩個輸入3D張量（rbot0 和 rbot1）之間的相關性，並將結果存儲在輸出張量 top 中。
# 應用於圖像處理、特徵匹配和深度學習中的相似性度量等場景。
...
# 計算一個特定張量（gradFirst）的梯度，該梯度是通過考慮輸入3D張量（rbot0 和 rbot1）及其相應的梯度輸出（gradOutput）來計算。
# 提高性能，特別是在處理高維數據時。這種計算通常用於訓練深度學習模型，特別是在需要進行反向傳播時。
...
# 計算一個名為 gradSecond 的輸出張量的梯度更新。這個更新是通過計算來自另一個輸入張量 rbot0 和 gradOutput 的值來實現。
# 高效處理多通道輸入數據，並計算出所需的梯度，這在深度學習模型的反向傳播過程中非常重要，並且這是優化過的計算過程，能夠有效地處理大規模的數據集
...
# 給定的 CUDA 核心函數名稱（strFunction）和變量（objVariables）來生成最終的 CUDA 核心代碼。
# 動態生成 CUDA 核心代碼，根據提供的張量大小和步長替換特定的常量和索引計算。
...
# 編譯生成的 CUDA 核心並返回可調用的函數。
@cupy.memoize(for_each_device=True)
def cupy_launch(strFunction, strKernel):
	# return cupy.cuda.compile_with_cache(strKernel).get_function(strFunction)
	# 使用 RawModule 來替代已廢棄的 compile_with_cache
    module = cupy.RawModule(code=strKernel)
    return module.get_function(strFunction)
# end

#  自動微分函數，並實現 forward 和 backward 方法。用於構建複雜的神經網絡模型，特別是在需要計算相關性或相似性的情況
class _FunctionCorrelation(torch.autograd.Function):
	@staticmethod
	def forward(self, first, second, intStride):
		# 儲存重新排列後的輸入數據，並依照 intStride 大小進行擴展
	...
	def backward(self, gradOutput):
		...
		# 根據需要計算的輸入梯度來初始化 gradFirst 和 gradSecond 張量
...
# 為一個獨立工具，是 _FunctionCorrelation 類的封裝，主要用於調用這個自定義的 PyTorch 自動微分函數。
def FunctionCorrelation(tenFirst, tenSecond, intStride):
	return _FunctionCorrelation.apply(tenFirst, tenSecond, intStride)	# 自動執行forward
# end

# 可以像其他 PyTorch 模組一樣進行組合，方便在更複雜的網絡中使用，但實際上跟 FunctionCorrelation 是一樣的
