...
opt.warproot = '/mnt/c/Users/User/Desktop/yein_VTON/sample/test_warping/result2/train'
opt.segroot = '/mnt/c/Users/User/Desktop/yein_VTON/sample/test_warping/seg2/train'
...
for epoch in range(start_epoch, opt.niter + opt.niter_decay + 1):
    ...
    for i, data in enumerate(tqdm(train_loader, desc="Training Tryon", total=len(train_loader))):
        ...
        if result_out:
            ...
            cv2.imwrite('/sample/train_tryon/ResUnet/result/'+str(epoch) + '_'+str(step)+'.jpg', bgr)
            result_out = False
    ...
    if epoch % opt.save_epoch_freq == 0:
      if opt.local_rank == 0:
        ...
        save_checkpoint(gen_model, os.path.join(opt.checkpoints_dir, opt.name, 'PBAFN_tryon_ResUnet_gen_epoch_%03d.pth' % (epoch+1)))
        save_checkpoint(discriminator, os.path.join(opt.checkpoints_dir, opt.name, 'PBAFN_tryon_ResUnet_D_epoch_%03d.pth' % (epoch+1)))
    if epoch > opt.niter:
